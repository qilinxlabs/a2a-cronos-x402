...
export class ContractReader {
     ...
     */
    async getSettlementRouter(hookAddress) {
        try {
     ...
     */
    async calculateCommitment(routerAddress, params) {
        try {
            const router = new ethers.Contract(routerAddress, SETTLEMENT_ROUTER_ABI, this.provider);
            const commitment = await router.calculateCommitment(params.token, params.from, params.value, params.validAfter, params.validBefore, params.salt, params.payTo, params.facilitatorFee, params.hook, params.hookData);
            return commitment;
        }
        catch (error) {
            if (error instanceof Error) {
